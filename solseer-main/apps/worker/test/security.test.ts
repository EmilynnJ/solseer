...
describe("API security boundaries", () => {
  ...
  it("does not follow redirects while downloading chat transcripts", async () => {
    vi.stubGlobal(
      "fetch",
    ...
    await expect(
      downloadLimitedJson("https://realtimekit.com/redirect", 1000),
  ...
  it("rejects excessive forum page offsets before querying the database", async () => {
    const response = await SELF.fetch(
