...
describe("API security boundaries", () => {
  ...
  it("rejects invalid UUID parameters early with INVALID_UUID", async () => {
    const response = await SELF.fetch(
      "https://api.example.test/api/readers/not-a-valid-uuid",
    );
    expect(response.status).toBe(400);
    const body = await response.json<{ error: { code: string; message: string } }>();
    expect(body.error.code).toBe("INVALID_UUID");
    expect(body.error.message).toBe("Invalid identifier format.");
  });
});
